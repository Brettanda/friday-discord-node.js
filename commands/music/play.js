    ...
    ytdl
      ...
      .catch((err) => {
        if (err.toString().includes("Not a YouTube domain") || err.toString().includes("No video id found:")) {
          ...
          console.error(err);
          func.msgDev(err, bot, "log-errors", msg, "Music Error");
          return msg.reply(func.embed({ title: "Something has gone wrong. Please try again later.", color: "#7BDCFC", author: msg.author }));
        }
      });
  },
};
...
async function play(msg, song) {
  ...
  const dispatcher = serverQueue.connection
    .play(
      ...
      { seek: 0, volume: 0.1, highWaterMark: 12 }
    )
    .on("finish", () => {
      console.log("finish");
    ...
    .on("error", (error) => {
      ...
      serverQueue.textChannel
        .send(func.embed({ title: "Something went wrong while connecting to the voice channel. Please try again later!", color: "#7BDCFC", author: msg.author }))
        .then((status) => {
