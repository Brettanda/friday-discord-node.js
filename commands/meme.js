...
module.exports = {
  ...
  async execute(msg, type) {
    let sub = ["dankmemes", "memes", "comedycemetery", "technicallythetruth", "rareinsults"];
    if (msg.channel.type != "dm") {
      const chName = msg.channel.name;
      if (!chName.includes("no") && !chName.includes("dont")) {
        if (chName.includes("anime") || chName.includes("weeb") || type == "anime") sub = ["animemes"];
        else if ((msg.channel.nsfw && chName.includes("hentai")) || type == "hentai") sub = ["hentaimemes"];
        else if (msg.channel.nsfw && (chName.includes("disturbing") || chName.includes("wtf") || type == "wtf")) sub = ["wtf", "creepy", "makemesuffer", "cursed_images"];
        else if (chName.includes("farmbuild") || chName.includes("minecraft") || type == "minecraft") sub = ["minecraftmemes"];
        else if (chName.includes("politics") || type == "politics") sub = ["historymemes"];
        else if (chName.includes("cool") || type == "cool") sub = ["nextlevel"];
      }
    }
